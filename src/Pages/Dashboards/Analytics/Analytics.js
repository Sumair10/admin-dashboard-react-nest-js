...
const Analytics = (props) => {
  ...
  useEffect(() => {
    ...
    socket.emit("fetchAttendanceFromServer", {
      schoolId: schoolId,
    });

    socket.on("returnAttendanceFromServer", (data) => {
      console.log('dataReturnAttendanceFromServer', data);
      setOnBoardStudents(data)
    });

    socket.emit("sendBusesDataToServer", {
      schoolId: schoolId,
      busId: busIds,
    });

    socket.on("getBusesLocationFromServer", (data) => {
