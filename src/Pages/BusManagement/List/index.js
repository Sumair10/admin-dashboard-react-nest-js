...
const List = (props) => {
  ...
  const structureData = () => {
    const remappedBus = buses.map((bus) => {
      return {
        busNo: bus.busNo,
        driver:
          bus.driverId && bus.currentlyAssignedDriver
            ? bus.driverId.name
            : "---",
        nanny:
  ...
  const handleUpload = async () => {
    ...
    console.log("buses.Sheet1", buses.in);
    for (const bus of buses.in) {
      if (
        bus["bus No(required)"] &&
        bus["Student Capacity(required)"] &&
        bus["VIN Number(required)"] &&
        parseInt(bus["Student Capacity(required)"])
      ) {
        formattedBus.push({
          schoolId,
            ...
            <Card className="main-card mb-3">
              <CardBody>
                {loader || loading ? (
                  ...
                  <div className="no-content">No Videos Present</div>
                )}
              </CardBody>
            </Card>
          </Col>
                  ...
                  {({ getRootProps, getInputProps }) => (
                    <div {...getRootProps()}>
                      ...
                      <div className="dropzone-content">
                        <p>
                          Upload an excel file which contains the data of Buses
                        </p>
                      </div>
                    </div>
                  )}
